...
class TripleDESFixedGUI:
    def __init__(self, root):
        self.root = root
        root.title("Triple DES — Encryption & Decryption Tool")
        ...
        # Canvas setup for scrolling (original code kept for brevity)
        ...
        self.notebook.add(self.encrypt_tab, text='🔑 Encryption (Steps)')
        self._setup_encryption_tab(self.encrypt_tab)

        # 2. DECRYPTION TAB
        self.decrypt_tab = ttk.Frame(self.notebook)
        self.notebook.add(self.decrypt_tab, text='🔓 Decryption (Independent)')
        self._setup_decryption_tab(self.decrypt_tab)

    # ----------------------------------------------------------------------------------
    # --- ENCRYPTION TAB SETUP (Modified for key input) ---
    # ----------------------------------------------------------------------------------
    def _setup_encryption_tab(self, parent_frame):
        
        # Controls: plaintext input, mode, generate keys, encrypt, show steps
        ctrl = ttk.Frame(parent_frame)
        ctrl.pack(fill=tk.X, pady=(6, 6), padx=4)

        # Row 0: Plaintext Input
        ...
        # Row 1: Key Input Fields
        key_input_frame = ttk.Frame(ctrl)
        key_input_frame.grid(row=1, column=0, columnspan=6, sticky=tk.W, pady=(8, 4))

        # K1
        ttk.Label(key_input_frame, text="K1 (Hex 16 chars):").grid(row=0, column=0, sticky=tk.W, padx=(0,4))
        self.entry_k1_enc = ttk.Entry(key_input_frame, width=18)
        self.entry_k1_enc.grid(row=0, column=1, padx=(0,10), sticky=tk.W)

        # K2
        ttk.Label(key_input_frame, text="K2 (Hex 16 chars):").grid(row=0, column=2, sticky=tk.W, padx=(0,4))
        self.entry_k2_enc = ttk.Entry(key_input_frame, width=18)
        self.entry_k2_enc.grid(row=0, column=3, padx=(0,10), sticky=tk.W)

        # K3
        ttk.Label(key_input_frame, text="K3 (Hex 16 chars):").grid(row=0, column=4, sticky=tk.W, padx=(0,4))
        self.entry_k3_enc = ttk.Entry(key_input_frame, width=18)
        self.entry_k3_enc.grid(row=0, column=5, padx=(0,10), sticky=tk.W)
        
        # Row 2: Controls and Mode
        ctrl2 = ttk.Frame(ctrl)
        ctrl2.grid(row=2, column=0, columnspan=6, sticky=tk.W, pady=(4, 0))

        # Key Generation/Mode
        self.btn_gen = ttk.Button(ctrl2, text="Generate Random Keys (Fill Fields)", command=self.generate_random_keys)
        self.btn_gen.grid(row=0, column=0, padx=(0, 10))

        ttk.Label(ctrl2, text="Mode:").grid(row=0, column=1, sticky=tk.W, padx=(10, 4))
        self.mode_var = tk.StringVar(value='3')
        ttk.Radiobutton(ctrl2, text="2-key", variable=self.mode_var, value='2').grid(row=0, column=2, sticky=tk.W)
        ttk.Radiobutton(ctrl2, text="3-key", variable=self.mode_var, value='3').grid(row=0, column=3, sticky=tk.W)

        # Action Buttons
        self.btn_encrypt = ttk.Button(ctrl2, text="Encrypt & Show Steps (Uses Keys Above)", command=self.run_encrypt)
        self.btn_encrypt.grid(row=0, column=4, padx=(20, 6))
        
        self.btn_key_steps = ttk.Button(ctrl2, text="Show Key Gen Steps (K1/K2/K3)", command=self.open_key_details_selector)
        self.btn_key_steps.grid(row=1, column=0, padx=(0, 6), pady=(4,0), sticky=tk.W) 
        
        self.btn_save = ttk.Button(ctrl2, text="Save Steps Output", command=self.save_output)
        self.btn_save.grid(row=1, column=4, padx=(20, 6), pady=(4,0))
        
        # Keys display small line (now uses the input fields as source)
        self.keys_display = ttk.Label(ctrl2, text="Enter keys or Generate Random Keys")
        self.keys_display.grid(row=2, column=0, columnspan=5, sticky=tk.W, pady=(4, 0))

        # Steps output area (scrolled text)
        steps_out_frame = ttk.LabelFrame(parent_frame, text="Step-by-step Output (detailed per-block / per-round)")
        steps_out_frame.pack(fill=tk.BOTH, expand=True, padx=4, pady=(0, 4))
    ...
    # ----------------------------------------------------------------------------------
    # --- DECRYPTION TAB SETUP (Reused from previous version) ---
    # ----------------------------------------------------------------------------------
    def _setup_decryption_tab(self, parent_frame):
        
        input_frame = ttk.Frame(parent_frame)
        input_frame.pack(fill=tk.X, pady=10, padx=10)

        # Row 0: Ciphertext Input
        ttk.Label(input_frame, text="Ciphertext (Hex):").grid(row=0, column=0, sticky=tk.W, pady=5)
        self.entry_cipher_dec = ttk.Entry(input_frame, width=80)
        self.entry_cipher_dec.grid(row=0, column=1, columnspan=3, padx=6, sticky=tk.W)
        
        # Row 1: Key Inputs K1, K2
        ttk.Label(input_frame, text="K1 (Hex 16 chars):").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.entry_k1_dec = ttk.Entry(input_frame, width=20)
        self.entry_k1_dec.grid(row=1, column=1, padx=6, sticky=tk.W)

        ttk.Label(input_frame, text="K2 (Hex 16 chars):").grid(row=1, column=2, sticky=tk.W, pady=5)
        self.entry_k2_dec = ttk.Entry(input_frame, width=20)
        self.entry_k2_dec.grid(row=1, column=3, padx=6, sticky=tk.W)

        # Row 2: Key Input K3, Mode
        ttk.Label(input_frame, text="K3 (Hex 16 chars):").grid(row=2, column=0, sticky=tk.W, pady=5)
        self.entry_k3_dec = ttk.Entry(input_frame, width=20)
        self.entry_k3_dec.grid(row=2, column=1, padx=6, sticky=tk.W)
        
        ttk.Label(input_frame, text="Mode:").grid(row=2, column=2, sticky=tk.W, pady=5)
        self.mode_var_dec = tk.StringVar(value='3')
        ttk.Radiobutton(input_frame, text="2-key (K3=K1)", variable=self.mode_var_dec, value='2').grid(row=2, column=3, padx=(0, 60), sticky=tk.W)
        ttk.Radiobutton(input_frame, text="3-key (K3!=K1)", variable=self.mode_var_dec, value='3').grid(row=2, column=3, padx=(90, 0), sticky=tk.W)

        # Row 3: Decrypt Button
        self.btn_decrypt_indep = ttk.Button(input_frame, text="Decrypt Ciphertext", command=self.run_independent_decrypt)
        self.btn_decrypt_indep.grid(row=3, column=0, columnspan=4, pady=10)
        
        # Output Area
        output_frame = ttk.LabelFrame(parent_frame, text="Decrypted Plaintext Output")
        output_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))
        self.txt_decrypted_out = scrolledtext.ScrolledText(output_frame, height=10, wrap=tk.WORD, font=("Courier", 10))
        self.txt_decrypted_out.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

    # ----------------------------------------------------------------------------------
    # --- UPDATED KEY GENERATION METHOD ---
    # ----------------------------------------------------------------------------------
    def generate_random_keys(self):
        ...
        # Update the encryption tab key fields
        self.entry_k1_enc.delete(0, tk.END)
        self.entry_k1_enc.insert(0, k1.hex().upper())
        self.entry_k2_enc.delete(0, tk.END)
        self.entry_k2_enc.insert(0, k2.hex().upper())
        self.entry_k3_enc.delete(0, tk.END)
        self.entry_k3_enc.insert(0, k3.hex().upper())
        
        # Store keys internally and update display (will read from fields on encrypt)
        self.keys = (k1, k2, k3)
        self.mode3 = (self.mode_var.get() == '3')
        self.key_generation_details = None
        self.update_keys_display()

        # Also update the decryption tab fields for convenience
        self.entry_k1_dec.delete(0, tk.END)
        self.entry_k1_dec.insert(0, k1.hex().upper())
        self.entry_k2_dec.delete(0, tk.END)
        self.entry_k2_dec.insert(0, k2.hex().upper())
        self.entry_k3_dec.delete(0, tk.END)
        self.entry_k3_dec.insert(0, k3.hex().upper())
        self.mode_var_dec.set(self.mode_var.get())
        
        messagebox.showinfo("Keys Generated", "New random keys have been generated and populated in the key input fields.")

    def update_keys_display(self):
        # Now reflects the values in the input fields
        k1_hex = self.entry_k1_enc.get().strip()
        k2_hex = self.entry_k2_enc.get().strip()
        k3_hex = self.entry_k3_enc.get().strip()
        mode3 = (self.mode_var.get() == '3')
        
        if k1_hex and k2_hex and k3_hex:
            s = f"K1={k1_hex}  K2={k2_hex}"
            if mode3:
                s += f"  K3={k3_hex}"
            else:
            ...
            self.keys_display.config(text="Key fields must contain 16 hex chars (8 bytes) each.")


    # ----------------------------------------------------------------------------------
    # --- UPDATED ENCRYPTION METHOD (Reads keys from input fields) ---
    # ----------------------------------------------------------------------------------
    def run_encrypt(self):
        ...
        if not k1_hex or not k2_hex or not k3_hex:
            messagebox.showerror("Key Error", "Please generate random keys or enter custom keys for K1, K2, and K3.")
            return
            
        try:
            if not all(len(h) == 16 for h in [k1_hex, k2_hex, k3_hex]):
                 raise ValueError("Keys must be 16 hexadecimal characters (8 bytes).")

            k1 = bytes.fromhex(k1_hex)
            k2 = bytes.fromhex(k2_hex)
            k3_original = bytes.fromhex(k3_hex)
            self.keys = (k1, k2, k3_original) # Store valid keys internally
            ...
            messagebox.showerror("Key/Input Error", f"Invalid hex input or length: {e}")
            return
        ...
        # Regenerate round keys for display (to match the captured key_generation_details structure)
        ...
        # --- Stage 1, 2, 3 block processing and detailed round steps (logic remains unchanged) ---
        for bi in range(0, len(data_p), 8):
            ...
            for rnum, rk in enumerate(rk2[::-1], 1): 
                expanded = tdb.permute(Rcur, tdb.E)
                xored = tdb.xor(expanded, rk)
                s_out = tdb.sbox_sub(xored)
                p_out = tdb.permute(s_out, tdb.P)
        ...
        self.txt_steps.insert(tk.END, self.generated_cipher.hex().upper() + "\n")
        self.txt_steps.configure(state='disabled')

        # FINAL STEP: Update the Decryption Tab with the new ciphertext and keys
        self.entry_cipher_dec.delete(0, tk.END)
        self.entry_cipher_dec.insert(0, self.generated_cipher.hex().upper())
        self.entry_k1_dec.delete(0, tk.END)
        self.entry_k1_dec.insert(0, k1_hex)
        self.entry_k2_dec.delete(0, tk.END)
        self.entry_k2_dec.insert(0, k2_hex)
        self.entry_k3_dec.delete(0, tk.END)
        self.entry_k3_dec.insert(0, k3_hex)
        self.mode_var_dec.set(self.mode_var.get())
        
        self.update_keys_display()
        messagebox.showinfo("Done", "Encryption completed. Ciphertext and keys are automatically copied to the Decryption tab.")


    # --- Rest of the helper methods (toggle_detail, _populate_tables_initial, etc.) remain unchanged ---
    ...
    def _populate_tables_initial(self):
        ...
        def grid_table(title, data_list):
            ...
            tlabel.pack(anchor=tk.W, pady=(3, 2), padx=4)
            gridf = ttk.Frame(tf)
    ...
    def run_decrypt_dialog(self):
        messagebox.showinfo("Use Decryption Tab", "Please use the 'Decryption (Independent)' tab for all decryption operations. Keys and Ciphertext were automatically copied after encryption.")

    def run_independent_decrypt(self):
        self.txt_decrypted_out.configure(state='normal')
        self.txt_decrypted_out.delete('1.0', tk.END)

        # 1. READ INPUTS
        cipher_hex = self.entry_cipher_dec.get().strip()
        k1_hex = self.entry_k1_dec.get().strip()
        k2_hex = self.entry_k2_dec.get().strip()
        k3_hex = self.entry_k3_dec.get().strip()
        mode3 = (self.mode_var_dec.get() == '3')
        
        output = ""
        
        try:
            if not cipher_hex or not k1_hex or not k2_hex or not k3_hex:
                 raise ValueError("All fields (Ciphertext, K1, K2, K3) must be filled.")

            # Validate and convert keys/cipher
            if not all(len(h) == 16 for h in [k1_hex, k2_hex, k3_hex]):
                 raise ValueError("Keys must be 16 hexadecimal characters (8 bytes).")
            if len(cipher_hex) % 16 != 0:
                 output += "Warning: Ciphertext length is not a multiple of 16 (8 bytes), decryption may fail.\n"

            cbytes = bytes.fromhex(cipher_hex)
            k1 = bytes.fromhex(k1_hex)
            k2 = bytes.fromhex(k2_hex)
            k3_original = bytes.fromhex(k3_hex)
            
            output += f"Mode: {'3-key (K3 used)' if mode3 else '2-key (K3=K1)'}\n"
            output += f"K1: {k1_hex}\nK2: {k2_hex}\nK3 Used: {k3_hex if mode3 else k1_hex}\n"
            output += f"Ciphertext: {cipher_hex}\n\n"

            # 2. CALL BACKEND DECRYPTION
            plain = tdb.triple_des_decrypt(cbytes, k1, k2, k3_original, mode3)
            
            # 3. DISPLAY RESULT
            try:
                text = plain.decode('utf-8').strip()
                output += "=== Decryption Successful (UTF-8) ===\n"
                output += text
            except Exception:
                output += "=== Decryption Successful (Raw Hex) ===\n"
                output += plain.hex().upper()
                output += "\n\n(Could not decode as standard UTF-8 text)"

        except ValueError as e:
            output = f"ERROR: Invalid input format: {e}"
        except Exception as e:
            output = f"ERROR: Decryption Failed. Check keys, mode, and ciphertext.\nDetail: {e}"
            
        self.txt_decrypted_out.insert(tk.END, output)
        self.txt_decrypted_out.configure(state='disabled')
